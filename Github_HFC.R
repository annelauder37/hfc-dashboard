...
server <- function(input, output, session) {
  ...
  duplicates_data <- reactive({
    if (input$type == "All Duplicate Entries") { 
      labor_el1 %>%
        group_by(anon_ID) %>%
        filter(n() > 1) %>%
        ungroup() %>%
        dplyr::select(anon_ID, anon_enum) %>%
        arrange(anon_ID)
    } else {
      labor_el1 %>%
        group_by(anon_ID) %>%
  ...
  output$download_duplicates <- downloadHandler(
    filename = function() { "duplicates.csv" },
    content = function(file) {
      write.csv(duplicates_data(), file, row.names = FALSE)
    }
  )
  ...
  output$skip <- renderPlot({
    labor_el1$skipped <- apply(labor_el1[, -1], 1, function(x) sum(is.na(x)))
  ...
  output$alternative<-renderDT({
    if (input$alt_options == "Percentage of surveys which include at least one occurrence of don't know or unsure grouped by enumerator") {
      ...
      alt <- labor_el1 %>%
        group_by(anon_enum) %>%
        summarize(
          TotalTargetCodes = sum(sapply(across(everything()), function(x) sum(x %in% target_codes, na.rm = TRUE))),
          TotalSurveys = n(),
          Percentage = round((TotalTargetCodes / TotalSurveys) * 100, 2)
        ) %>%
